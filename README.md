...
- **本番（予定）**: https://caslive.app/game/（金さん側で配置運用、サブパス）
...
CORS allowlist（`ALLOWED_ORIGINS` 環境変数）: `kento-nakamura-gh.github.io` / `caslive.app` / localhost。新環境（ステージング等）追加時は `wrangler.toml` 編集 → `npx wrangler deploy`。
...
本番（`caslive.app/game/`）への反映は金さんが Release または main の最新を pull して配置する運用。中村側は `kento-nakamura-gh.github.io/game/` がプレビューとして常に最新を映す。
